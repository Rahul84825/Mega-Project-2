    ...
    <div className="rounded-3xl border border-[#e6d3b3] bg-[#fffaf3] shadow-sm">
      <div className="flex items-center justify-between border-b border-[#e6d3b3] px-6 py-5">
        <div>
          ...
          <div className="text-lg font-extrabold text-[#2d1b0e]">#{order.orderNumber || order.orderId}</div>
        </div>
        {onClose && (
          <button onClick={onClose} className="text-xs font-semibold text-[#8b4513]">
            Close
          </button>
        )}
      </div>
      ...
      <div className="space-y-6 px-6 py-6">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <span className={`rounded-full px-3 py-1 text-xs font-semibold ${paymentStatusMeta.cls}`}>
        ...
        <div className="rounded-2xl border border-[#f0e0c4] bg-[#fff8ec] p-4">
          ...
          {delivery.trackingUrl && (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="mt-2 inline-flex text-xs font-semibold text-[#8b4513]"
            >
              Track order
            </a>
          )}
        ...
        <div className="rounded-2xl border border-[#f0e0c4] bg-[#fff8ec] p-4">
          ...
          <div className="mt-2 flex flex-wrap gap-2 text-xs">
            <button
              ...
              className="rounded-full border border-[#d4a373] px-3 py-1 font-semibold text-[#8b4513]"
            >
              Resend OTP
            </button>
            <button
              ...
              className="rounded-full bg-[#8b4513] px-3 py-1 font-semibold text-white"
            >
              Verify OTP
            </button>
          </div>
        ...
        <div className="rounded-2xl border border-[#f0e0c4] bg-[#fff8ec] p-4">
          <div className="text-xs font-semibold text-[#8b4513]">Order Items</div>
          <div className="mt-3 space-y-2 text-sm">
            {items.map((item) => (
              <div key={item._id || item.productId} className="flex items-start justify-between gap-3">
                <div>
                  <div className="font-semibold text-[#2d1b0e]">{item.titleSnapshot || item.name}</div>
                  <div className="text-xs text-[#6d4c41]">
                    {item.selectedVariant?.label || "Standard"} · Qty {item.quantity}
                  </div>
                </div>
                <div className="font-semibold text-[#2d1b0e]">{formatPrice(item.finalAmount || item.price || 0)}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-[#f0e0c4] bg-[#fff8ec] p-4">
          <div className="text-xs font-semibold text-[#8b4513]">Pricing</div>
          <div className="mt-3 space-y-2 text-sm text-[#6d4c41]">
            ...
            <div className="flex justify-between font-bold text-[#2d1b0e]">
