...
const AdminLayout = () => {
  ...
  const Sidebar = ({ mobile = false }) => (
    <div className={`flex flex-col h-full bg-[#f5e6d3] border-r border-[#e6d3b3] shadow-[4px_0_24px_rgba(0,0,0,0.25)] z-20 ${mobile ? "w-[280px]" : collapsed ? "w-[80px]" : "w-[260px]"} transition-all duration-300`}>
        ...
        {NAV_ITEMS.map((item) => {
          ...
          return (
            ...
            >
              ...
              <Icon className={`w-5 h-5 flex-shrink-0 transition-colors ${active ? "text-white" : "text-[#7a5c3a] group-hover:text-[#2d1b0e]"}`} strokeWidth={active ? 2.5 : 2} />
      ...
      <div className="px-4 py-5 border-t border-[#e6d3b3] space-y-2 bg-[#f5e6d3]">
        <button
          onClick={() => navigate("/")}
          className={`flex items-center gap-3 ${collapsed && !mobile ? 'justify-center w-10 h-10 mx-auto px-0' : 'w-full px-3 py-2.5'} rounded-xl text-sm font-semibold text-[#7a5c3a] hover:bg-[#d4a373] hover:text-[#8b4513] border border-transparent hover:border-[#e6d3b3] transition-all`}
        ...
        <button
          ...
          className={`flex items-center gap-3 ${collapsed && !mobile ? 'justify-center w-10 h-10 mx-auto px-0' : 'w-full px-3 py-2.5'} rounded-xl text-sm font-semibold text-red-600 hover:bg-[#d4a373] transition-all`}
      ...
      <div className="flex-1 flex flex-col overflow-hidden relative z-10">
        ...
        <header className="bg-[#fffaf3] border-b border-[#e6d3b3] px-4 h-16 flex items-center gap-4 flex-shrink-0 shadow-sm md:hidden">
          ...
          <div className="flex items-center gap-2">
            <div className="w-6 h-6 bg-gradient-to-br from-[#8b4513] to-[#d4a373] rounded-lg flex items-center justify-center">
              <span className="text-white font-bold text-[10px]">M</span>
            </div>
            <h1 className="text-sm font-bold text-[#2d1b0e] tracking-tight">Admin</h1>
          </div>
