...
const AdminMalaiBarfiProducts = () => {
  ...
  const filtered = useMemo(() => {
    ...
    return (products || [])
      .filter((p) => {
        return (p.name || "").toLowerCase().includes('malai barfi');
      })
      .filter((p) => {
        if (!q) return true;
      ...
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 bg-white p-6 md:p-8 rounded-[32px] border border-[#e6d3b3] shadow-sm">
        <div>
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#fffaf3] text-[#b67b3a] border border-[#f0e0c4] text-[10px] font-bold uppercase tracking-widest mb-4">
            <LayoutGrid size={14} /> Specialty Barfi
          </div>
          <h2 className="serif text-3xl md:text-4xl font-medium text-[#2d1b0e] mb-2">Malai Barfi Collection</h2>
          <p className="text-sm font-medium text-[#7a5c3a]">View and manage your Malai Barfi variations.</p>
        </div>
      </div>
              ...
              {filtered.length === 0 ? (
                <tr>
                  <td colSpan={4} className="px-6 py-24 text-center">
                    ...
                    <p className="text-sm font-bold text-[#7a5c3a]">No Malai Barfi products found.</p>
                  </td>
                </tr>
              ) : (
...
export default AdminMalaiBarfiProducts;