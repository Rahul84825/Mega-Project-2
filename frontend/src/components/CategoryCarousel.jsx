        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 sm:gap-6">
          {homepageCategories.map((category, idx) => (
            <button
              ...
              className="group relative w-full h-56 sm:h-72 lg:h-80 overflow-hidden rounded-[24px] sm:rounded-[32px] border border-[var(--surface-border)] shadow-sm hover:shadow-xl transition-all duration-700 hover:-translate-y-1.5 active:scale-95 animate-in fade-in slide-in-from-bottom-4"
            >
              ...
              <div className="absolute inset-0 flex flex-col justify-end p-5 sm:p-6 z-10 text-left">
                <h3 className="serif text-white font-bold text-lg sm:text-xl lg:text-2xl drop-shadow-lg leading-tight transition-transform duration-500 group-hover:-translate-y-1">
                  {category.name}
                </h3>
                <div className="w-8 h-0.5 bg-[var(--saffron)] mt-3 transition-all duration-700 ease-out group-hover:w-full group-hover:bg-[var(--gold)] shadow-[0_0_10px_rgba(212,160,23,0.5)]" />
                
                <div className="flex items-center gap-2 text-[8px] sm:text-[9px] font-black uppercase tracking-[0.2em] text-white/90 mt-4 opacity-0 group-hover:opacity-100 transition-all translate-y-2 group-hover:translate-y-0 duration-500 delay-100">
                  Shop Now <ArrowRight size={12} className="text-[var(--gold)]" />
                </div>
              </div>
